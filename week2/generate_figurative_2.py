...
general_categories = tr.member(source)
#print(general_categories.keys())
categories=general_categories.get('category_heads')
#print(categories)
selected_category=(Counter(categories).most_common(3))[0][0]
...
property=(Counter(extracted_modifiers).most_common(3))[0][0]
print('selectedModifier:', property)
new_concepts = tr.category(property, selected_category)
...
newer_concepts=tr.category(property, new_category)
print(newer_concepts)
print(newer_concepts.keys())
newer_concepts=newer_concepts.get('category_heads')
target=(Counter(newer_concepts).most_common(3))[0][0]
print('Final Concept:', target)
print('FIGURATIVE SENTENCE:', target, ' is as ', property, ' as ', source)

# random category containing target concept
target_categories=tr.member(target)
t_categories=target_categories.get('category_heads')
#print(t_categories)
t_category=random.choice(list(t_categories.keys()))
while(list(categories.keys()).__contains__(t_category)==False):
    t_category=random.choice(list(t_categories.keys()))
    #print(list(categories.keys()).__contains__(t_category))
#print(t_category)
t_property=random.choice(list(t_categories.keys()))
#print('target Modifier:', t_property)
print('case 1: FIGURATIVE SENTENCE:', target, ' is as ', t_property, ' as ', source)

# random category not containing target concept
# random category containing target concept
target_categories=tr.member(target)
t_categories=target_categories.get('category_heads')
#print(t_categories)
t_category=random.choice(list(t_categories.keys()))
while(list(categories.keys()).__contains__(t_category)==True):
    t_category=random.choice(list(t_categories.keys()))
    #print(list(categories.keys()).__contains__(t_category))
#print(t_category)
t_property=random.choice(list(t_categories.keys()))
#print('target Modifier:', t_property)
print('case 2: FIGURATIVE SENTENCE:', target, ' is as ', t_property, ' as ', source)